...
export default function Tokenpairs(props) {
  ...
  for (let i = 0; i < props.token.pairs.length; i++) {
    ...
    var pairlink =
      "https://v2.kaidex.io/exchange/" + props.token.pairs[i].pairID;
    pairs.push(
      <a
        key={i}
        className="t-d-none"
      ...
      >
        <div className="pairstats">
          <div className="pair-name">
            ...
            <span className="fs-14 p-l-12 p-r-25">
              {props.token.pairs[i].pairName}
            </span>
          </div>
          <span className="fs-14 m-t-10 m-b-10">
            TVL: $
            {numberWithCommas(
              parseFloat(props.token.pairs[i].pairTVL).toFixed(2)
            )}
          </span>
        </div>
      </a>
  ...
  return (
    <div className={"section pairsec " + props.cn}>
      <div className="fs-14 p-t-15 p-b-15 p-l-15 bor">Pairs:</div>
      {pairs}
    </div>
  );
}

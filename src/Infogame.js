...
import gameABI from "./ABI/betTKN.json";
...
class Game extends Component {
  ...
  handleBet(event) {
    ...
    if (event.nativeEvent.inputType !== "deleteContentBackward") {
      if (w > this.weiToEth(this.state.tknBal)) {
        w = roundDown(this.weiToEth(this.state.tknBal, 4) - 0.1, 4);
      }
      if (m * w >= this.state.maxWin) {
        w = this.state.maxWin / m;
      }
  ...
  handleBM(event) {
    ...
    if (w > this.weiToEth(this.state.tknBal) - 0) {
      w = roundDown(this.weiToEth(this.state.tknBal, 4) - 0, 4);
    }
    if (m * w >= this.state.maxWin) {
      w = this.state.maxWin / m;
    }
  ...
  handleMaxBet() {
    var w = roundDown(this.weiToEth(this.state.tknBal, 4) - 0, 4);
    var m = this.state.multiplier;
  ...
  async handlePlay(event) {
    if (
      ...
      if (event.target.innerText === "Bet High") {
        ...
        g = 0;
        this.setState({ tr: "lo" });
      } else if (event.target.innerText === "Approve") {
        g = 2;
        this.setState({ tr: "appr" });
      }
      if (g < 2) {
        var val = this.state.web3.utils.toWei(
          this.state.wager.toString(),
          "ether"
        );
        var m = Math.round(this.state.multiplier * 100);

        if (this.state.wager > this.state.tknBal) {
        ...
        await this.state.game.methods
          ...
          .on(
            ...
            function (receipt) {
              ...
              var tknBal, num, gameres;
              if (receipt.win === false) {
                console.log("lost");
                tknBal =
                  parseFloat(this.state.web3.utils.fromWei(this.state.tknBal)) -
                  parseFloat(this.state.web3.utils.fromWei(receipt.wager));
                console.log(tknBal);
                tknBal = this.state.web3.utils.toWei(
                  tknBal.toString(),
                  "ether"
                );
                console.log(tknBal);
              ...
              console.log(tknBal, num, gameres);
              this.setState({
                hasPlayed: true,
                tr: "",
                tknBal,
        ...
        console.log(this.state.gameAddr);
        await this.state.tkn.methods
          .approve(
            this.state.gameAddr,
  ...
  async refreshData() {
    ...
    if (this.state.tr === "") {
      try {
        ...
        var gameAddr = "0x1Ab992A59C5A253A6c28eBC9649407FDbBb4f6E4";
        var tokenAddr = "0x5FFD7a138422cBbcfB53908AD51F656D7C6c640F";
        var game, tkn;

        if (!this.state.game) {
          game = new web3.eth.Contract(gameABI, gameAddr);
          game = new web3.eth.Contract(gameABI, gameAddr);
          tkn = new web3.eth.Contract(tokenABI, tokenAddr);
        } else {
        ...
        let [tknBal, maxWin, gL, appr] = await Promise.all([
          tkn.methods.balanceOf(accounts[0]).call(),
          game.methods.maxWin().call(),
          game.methods.gamesL(accounts[0]).call(),
          tkn.methods.allowance(accounts[0], gameAddr).call(),
        ]);
        maxWin = web3.utils.fromWei(maxWin);
        ...
        var hi, lo, actuallyApproved;
        if (this.state.actuallyApproved === true) {
          actuallyApproved = true;
        }
        if (
          this.state.actuallyApproved === true &&
              ...
              <div className="bottom game bor p-b-10 p-t-15 m-b-15">
                ...
                <div className="gameinput  m-b-20">
                  <div className="tophalf m-b-5">
                    ...
                    >
                      Balance:{" "}
                      {this.niceNum(this.state.tknBal, 2, 1) +
                        " " +
                        this.state.tokenName}
                    </span>
                  </div>
                      ...
                      >
                        <MenuItem value={0}>
                          <div className="d-flex al-c">
                            <img
                              className="gameminiimg INFOsml m-r-10"
                              src="img/smalllogo.png"
                            />
                            INFO
                          </div>
                        </MenuItem>
                        <Link className="t-d-none" to="/game">
                          <MenuItem value={20}>
                            <div className="d-flex al-c">
                              <img
                                ...
                                src="img/KAI.png"
                              />
                              <span className="t-d-none">KAI</span>
                            </div>
                          </MenuItem>
                        </Link>
                      </Select>
                ...
                <div className="gameinput b m-b-10">
                  ...
                  <div className="gameBtns m-t-20">
                    <button
                      disabled={this.state.disabled}
                      className={"btn hi r " + this.state.lo}
                    ...
                    <button
                      disabled={this.state.disabled}
                      className={"btn lo l " + this.state.hi}
                      onClick={this.handlePlay}
                    >
                      {this.state.hi}{" "}
                      <img
                ...
                <div className="gamean">
                  ...
                  <div>
                    ...
                    <p className="txt-r fs-24 f-ws m-t-8 m-b-0">
                      ...
                      <span className="t-s">{this.state.tokenName}</span>
                    </p>
                  </div>
                </div>
              </div>
